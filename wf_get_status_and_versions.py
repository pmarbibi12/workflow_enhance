...
def get_statuses(dataowner, env, workflowId, auth):
...
def statusId_to_statusName(step_id, steps_with_names):
...
# def getStatuses(productGuids, steps_with_names, env, workflowId, auth):   
#     timeStamp = []
#     wf_version = []
#     instaceIds = []
...
#                 name = statusId_to_statusName(status, steps_with_names)
...
async def fetch_status(session, product, steps_with_names, env, workflowId, auth):
    try:
        status_search_url = f"https://{env}/api/workflow/WorkflowGrain/status/{workflowId}/{product}"
        headers = get_headers()
        headers["Authorization"] = auth

        async with session.get(status_search_url, headers=headers) as response:
            if response.status == 200:
                json_response = await response.json()
                timestamp_response = json_response["Timestamp"]
                status_response = json_response["Statuses"]
                stat_names = [statusId_to_statusName(status, steps_with_names) for status in status_response]
                return {
                    "Product ID": product,
        ...
        print(f"Error for product {product}: {e}")
        return None

async def getStatuses(productGuids, steps_with_names, env, workflowId, auth, session):
    tasks = [fetch_status(session, product, steps_with_names, env, workflowId, auth) for product in productGuids]
    results = await asyncio.gather(*tasks)
    return results
    
async def create_session():
    return aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit_per_host=10))
